    ...
    <div class="auth-card-wrapper">
      <div class="auth-card">
        <div class="card-header">
          ...
          <p class="card-subtitle">输入邮箱重置密码</p>
        </div>
        ...
        <n-form ref="formRef" class="auth-form" :model="formData" :rules="rules" :show-label="false">
          <n-form-item path="email">
            <div class="input-wrapper">
              ...
              <n-input v-model:value="formData.email" placeholder="请输入注册邮箱" @keyup.enter="handleSubmit" />
            </div>
          </n-form-item>

          <n-form-item style="margin-top: 16px;">
            <n-button type="primary" block class="auth-btn" :loading="loading" @click="handleSubmit">
              <template #icon><n-icon :size="18"><SendOutlined /></n-icon></template>
              发送重置邮件
            </n-button>
          </n-form-item>
        </n-form>

        <div class="divider"><span class="divider-text">想起密码了？</span></div>
...
import { CompassOutlined, MailOutlined, SendOutlined } from '@vicons/antd'
