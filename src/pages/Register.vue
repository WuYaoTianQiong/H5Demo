    ...
    <div class="auth-card-wrapper">
      <div class="auth-card">
        <div class="card-header">
          ...
          <p class="card-subtitle">开始您的旅程</p>
        </div>
        ...
        <n-form ref="formRef" class="auth-form" :model="formData" :rules="rules" :show-label="false">
          <n-form-item path="username">
            <div class="input-wrapper">
              <div class="input-icon">
                <n-icon :size="18" color="#ff8e53"><UserOutlined /></n-icon>
              </div>
          ...
          <n-form-item path="email">
            <div class="input-wrapper">
              <div class="input-icon">
                <n-icon :size="18" color="#ff8e53"><MailOutlined /></n-icon>
              </div>
          ...
          <n-form-item path="password">
            <div class="input-wrapper">
              ...
              <n-input v-model:value="formData.password" :type="showPassword ? 'text' : 'password'" placeholder="请输入密码">
                <template #suffix>
                  <n-icon :size="18" :component="showPassword ? EyeOutlined : EyeInvisibleOutlined" @click="togglePassword('password')" style="cursor: pointer; color: #999;" />
                </template>
              </n-input>
            </div>
          </n-form-item>
          ...
          <n-form-item path="confirmPassword">
            <div class="input-wrapper">
              ...
              <n-input v-model:value="formData.confirmPassword" :type="showConfirmPassword ? 'text' : 'password'" placeholder="请确认密码" @keyup.enter="handleRegister">
                <template #suffix>
                  <n-icon :size="18" :component="showConfirmPassword ? EyeOutlined : EyeInvisibleOutlined" @click="togglePassword('confirm')" style="cursor: pointer; color: #999;" />
                </template>
              </n-input>
            </div>
          </n-form-item>
          ...
          <n-form-item style="margin-top: 16px;">
            <n-button type="primary" block class="auth-btn" :loading="loading" @click="handleRegister">
              <template #icon><n-icon :size="18"><UserAddOutlined /></n-icon></template>
              立即注册
            </n-button>
          </n-form-item>
        </n-form>

        <div class="divider"><span class="divider-text">已有账号？</span></div>

        <n-button block class="secondary-btn" @click="goToLogin">返回登录</n-button>
      </div>
    </div>
...
import { CompassOutlined, UserOutlined, MailOutlined, LockOutlined, SafetyOutlined, UserAddOutlined, EyeOutlined, EyeInvisibleOutlined } from '@vicons/antd'
...
.divider::before, .divider::after { content: ''; flex: 1; height: 1px; background: linear-gradient(90deg, transparent, #e0e0e0); }
.divider::after { background: linear-gradient(90deg, #e0e0e0, transparent); }

.divider-text { padding: 0 12px; font-size: 13px; color: #999; white-space: nowrap; }

.secondary-btn { height: 42px; font-size: 15px; font-weight: 500; border-radius: 10px; border: 1.5px solid #ffccbc; color: #ff6b6b; background: transparent; transition: all 0.3s ease; }

.secondary-btn:hover { border-color: #ff6b6b; background: rgba(255, 107, 107, 0.05); }
